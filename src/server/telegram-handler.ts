// Shared Telegram update handler — used by both the global bot webhook
// (/api/telegram/webhook) and per-user bot webhooks
// (/api/telegram/webhook/[botKey]). The bot token is passed explicitly.

import { NextRequest, NextResponse } from 'next/server';
import { sendMessage, sendChatAction, getFile, downloadFile, type TgUpdate } from '@/services/telegram';
...
export async function handleTelegramUpdate(request: NextRequest, botToken: string): Promise<NextResponse> {
    let update: TgUpdate;
    try {
    ...
    try {
        ...
        if (text.startsWith('/start')) {
            ...
            if (!consume.success || !consume.uid) {
                await sendMessage(chatId, `❌ ${consume.error || 'Could not link. Try again.'}`, {}, botToken);
                return NextResponse.json({ ok: true });
            }

            await linkTelegramChat(consume.uid, String(chatId));
            await sendMessage(chatId, '✅ Account linked! You can now chat with your KhataHouse AI, add expenses, and upload documents.\n\nSend /help for commands.', {}, botToken);
            return NextResponse.json({ ok: true });
        }

        if (text === '/unlink') {
            await unlinkTelegramChat(String(chatId));
            await sendMessage(chatId, '🔗 Chat unlinked from KhataHouse.', {}, botToken);
            return NextResponse.json({ ok: true });
        }

        if (text === '/help') {
            await sendMessage(chatId, BOT_HELP, {}, botToken);
            return NextResponse.json({ ok: true });
        }
        ...
        if (message.document || (message.photo && message.photo.length > 0)) {
            await sendChatAction(chatId, 'upload_document', botToken);
            if (!uid) {
                await sendMessage(chatId, '🔒 Link your account first: send /start <code> (code from Settings → Link Telegram).', {}, botToken);
                return NextResponse.json({ ok: true });
            }
            ...
            if (!fileId) {
                await sendMessage(chatId, '❌ Could not read that file.', {}, botToken);
                return NextResponse.json({ ok: true });
            }

            const file = await getFile(fileId, botToken);
            if (!file.file_path) {
                await sendMessage(chatId, `❌ ${file.error || 'Failed to download file.'}`, {}, botToken);
                return NextResponse.json({ ok: true });
            }

            const download = await downloadFile(file.file_path, botToken);
            if (!download.buffer) {
                await sendMessage(chatId, '❌ Failed to download the file.', {}, botToken);
                return NextResponse.json({ ok: true });
            }

            await sendMessage(chatId, '📄 Analyzing document… (may take a minute)', {}, botToken);
            ...
            if (!result.success || !result.analysis) {
                await sendMessage(chatId, `❌ ${result.error || 'Could not analyze the document.'}`, {}, botToken);
                return NextResponse.json({ ok: true });
            }
        ...
        if (text.startsWith('/add-expense') || text.startsWith('/add')) {
            ...
            if (!rest) {
                await sendMessage(chatId, 'Usage: /add-expense <amount> <description>\nExample: /add-expense 250 lunch with team', {}, botToken);
                return NextResponse.json({ ok: true });
            }
            const amountMatch = rest.match(/^([\d,.]+)\s*(.*)$/);
            if (!amountMatch) {
                await sendMessage(chatId, 'Could not read the amount. Example: /add-expense 250 lunch', {}, botToken);
                return NextResponse.json({ ok: true });
            }
            const amount = parseFloat(amountMatch[1].replace(/,/g, ''));
            const description = amountMatch[2] || 'Telegram expense';
            if (isNaN(amount) || amount <= 0) {
                await sendMessage(chatId, 'Invalid amount.', {}, botToken);
                return NextResponse.json({ ok: true });
            }
            ...
            await sendMessage(chatId, result.text || 'Expense recorded.', {}, botToken);
            return NextResponse.json({ ok: true });
        }
        ...
        if (text.startsWith('/summary')) {
            ...
            await sendMessage(chatId, result.text || 'Could not fetch summary.', {}, botToken);
            return NextResponse.json({ ok: true });
        }
        ...
        if (text.startsWith('/portfolio')) {
            ...
            await sendMessage(chatId, result.text || 'Could not fetch portfolio.', {}, botToken);
            return NextResponse.json({ ok: true });
        }
        ...
        if (text.startsWith('/council')) {
            ...
            if (!question) {
                await sendMessage(chatId, 'Usage: /council <question>\nExample: /council Should I invest in crypto?', {}, botToken);
                return NextResponse.json({ ok: true });
            }
            await sendMessage(chatId, '🏛️ Convening the AI council… (up to a minute)', {}, botToken);
            ...
            if (!result.success || !result.data) {
                await sendMessage(chatId, `❌ Council failed: ${result.error}`, {}, botToken);
                return NextResponse.json({ ok: true });
            }
            const d = result.data;
            ...
            await sendMessage(chatId, `🏛️ Council Verdict\n\n${votes}\n\n${d.synthesis.verdict}${consensus}${actions}`, {}, botToken);
            return NextResponse.json({ ok: true });
        }
        ...
        if (text.startsWith('/chat')) {
            ...
            if (!rest) {
                await sendMessage(chatId, 'Usage: /chat <message>', {}, botToken);
                return NextResponse.json({ ok: true });
            }
            const result = await runServerChat({ uid, message: rest });
            await sendMessage(chatId, result.text || 'Sorry, I could not process that.', {}, botToken);
            return NextResponse.json({ ok: true });
        }
        ...
        try {
            await sendMessage(chatId, '⚠️ Something went wrong processing your message. Try again.', {}, botToken);
        } catch {
            // ignore
        }
        return NextResponse.json({ ok: true });
